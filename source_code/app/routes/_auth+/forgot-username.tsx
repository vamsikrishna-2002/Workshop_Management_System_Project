...
import { generateUsernameResetToken } from "~/utils/misc.server";
import { type inferErrors, validateAction } from "~/utils/validation";

const Schema = z
  .object({
    intent: z.literal("send_code").or(z.literal("verify_otp")),
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "send_code") {
    const [admin, participant, instructor] = await Promise.all([
      ...
      db.participant.findUnique({ where: { email } }),
      db.instructor.findUnique({ where: { email } }),
    ]);

    const user = admin || participant || instructor;

    if (!user) {
      return json({ fieldErrors: { email: "No account found with this email" } }, { status: 400 });
    }

    const token = generateUsernameResetToken();
    const expiresAt = new Date(Date.now() + 1000 * 60 * 15); // 15 minutes

    if (admin) {
      await db.usernameReset.create({
        data: {
          adminId: admin.id,
          token,
          expiresAt,
    ...
    await sendEmail({
      ...
      text: `Your username reset code is ${token}. It expires in 15 minutes.`,
    });
  ...
  if (intent === "verify_otp") {
    try {
      ...
      return redirect(`/reset-username?token=${otp}`);
    } catch (error) {
      ...
      <div className="mb-12 text-center">
        ...
        <Text className="mt-2 text-slate-600">
          {fetcher.data?.success
            ...
            : "We'll send you instructions to recover your username"}
        </Text>
      </div>

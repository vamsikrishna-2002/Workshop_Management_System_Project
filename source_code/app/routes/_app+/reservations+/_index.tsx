...
export default function InstructorSessionsPage() {
  ...
  const RegisteredSessions = registrations.map((reg) => reg.session);

  return (
    <div className="container mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">View All Reserved Sessions</h1>
        <p className="mt-1 text-sm text-gray-500">View all the sessions you have reserved</p>
      </div>

      {RegisteredSessions.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-gray-300 p-12">
          <div className="text-center">
            <h3 className="mt-2 text-sm font-semibold text-gray-900">No sessions reserved</h3>
            <p className="mt-1 text-sm text-gray-500">
              There are no sessions reserved at the moment.
            </p>
          </div>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="inline-flex items-center rounded-full bg-purple-100 px-2 py-1 text-xs font-medium text-purple-700">
                      Session{" "}
                      {session!.workshop.sessions.findIndex((s) => s.id === session!.id) + 1} of{" "}
                      {session!.workshop.sessions.length}
                    </span>
                  </div>
                  <div>
                    <Link
                      to={`/reservations/${session!.id}/view`}
